...
def main():
    ...
    sim_match = sum(get_match(get_birthdays(number_birth_day))
                    is not None for _ in range(NUM_SIMS))
    elapsed_time = timeit.default_timer() - start_time

    probability = round(sim_match / NUM_SIMS * 100, 2)
    print(
        f'Out of {NUM_SIMS} simulations of {number_birth_day} people, there was a')
